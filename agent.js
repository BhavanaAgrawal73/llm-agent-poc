/**
 * Bridge_of_Knowledge - Advanced AI Assistant
 * v2.8.0 (Updated: AI Pipe integration + bug fixes)
 * Author: Bhavana Agrawal (Original) & Assistant (fixes)
 */
class Bridge_of_Knowledge {
    constructor() {
        this.version = '2.8.0';
    ...
    async init() {
        try {
            ...
            console.log(`Bridge_of_Knowledge v${this.version} initialized successfully`);
        } catch (error) {
            console.error('Failed to initialize Bridge_of_knowledge:', error);
    ...
    displayMessage(message) {
        try {
            ...
            const senderName = { user: 'You', assistant: 'Bridge_of_knowledge', system: 'System', tool: 'Tool' }[message.role] || message.role;
    ...
    showWelcomeMessage() {
        ...
        if (conv && conv.messages.length === 0) {
            this.addMessage('assistant', 'Welcome to Bridge_of_Knowledge! How can I assist you?');
        }
    }
...
document.addEventListener('DOMContentLoaded', () => {
    window.agentFlow = new Bridge_of_Knowledge();
});